const hoursTableEl = $("#hours"); // Get the table where the tasks are stored

let currentTime = moment(); // Get the current time

let currentDay = moment().format("dddd, MMMM Do"); // Get the current day
$("#currentDay").text(currentDay); // Put the current day in the header
...
if (!localStorage.getItem("schedule")) {
  // If there is no current schedule in local memory, save the empty schedule
...
let timer = setInterval(function () {
  // Update every 10 seconds to see if the hour has changed and if any of the table row's colours need to be updated
  currentTime = moment();
  for (let i = 0; i < 9; i++) {
...
for (let i = 0; i < 9; i++) {
  // Loop through the rows for the first time
...
hoursTableEl.on("click", ".saveBtn", function () {
